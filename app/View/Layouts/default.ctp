...
*/
    ...
    //$nonce1 = '6e19b093';   // = 1847177363 (decimal)  ->  2 million less than the required/correct nonce value.   [Other value pairs:  0x6e28f2d3 = 1848177363 (decimal) ]
      $nonce1 = '6e321a93';   // [Value pairs:  0x6e28f2d3 = 1848177363 (decimal) .  0x6e19b093 = 1847177363 (decimal) .  0x6e36ae73 = 1849077363 (decimal).  0x6e3527d3 = 1848977363 (decimal).  0x6e321a93 = 1848777363 (decimal)].
      $nonce2 = '0';   //I have tested with real PHP experiment on RedHat OpenShift platform with zero 'nonce2' value, and my PHP program below can really terminate/exit the 'do-while' loop properly.
...
//    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^  computation for 1st (i=0) -> 64th (i=63) round, third-stage :   ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
      ...
      if ($i==60) {   //(i==60)'s output reg_e will become (i==63)'s output reg_h.
        if (($reg_e+0x5be0cd19)&0xffffffff) {break;}  //[VERY IMPORTANT]: MUST have ending semicolon at   "break;"  . If not, RedHat OpenShift PHP interpreter will give error.
      ...
      if ($i==61) {   //(i==61)'s output reg_e will become (i==63)'s output reg_g.
        if (($reg_e+0x1f83d9ab)&0xffffffff) {break;}  //[VERY IMPORTANT]: MUST have ending semicolon at   "break;"  . If not, RedHat OpenShift PHP interpreter will give error.
      }
...
//} while (m[3]!=1949177363)   // =0x742e1613.  For experiment purpose (better use hex number here, because Javascript maximum POSITIVE decimal integer/number is until 2,147,483,647 only).
    ...
    echo "DONE!<br>";  //I have run real PHP experiment on RedHat OpenShift platform, and found that this message is NOT printed out if  'exit(0)' is used.  If  'exit(0)'  is used, only the correct/required nonce value is displayed.
                       //If  "break 2;"  is used instead of  "exit(0)" , then BOTH the nonce value AND this message will be printed out.
